"""
EME Mechanical Change Detector v10.0
Pure visual measurement instrument with attribution readiness
Outputs: Raw measurements with absolute timestamps, calibrated thresholds
"""
...
class Delta:
"""Immutable measurement record with absolute timestamp - pure data, no interpretation"""
change_type: ChangeType
coordinates: Tuple[int, int, int, int]  # x, y, w, h (pixel units)
measurement_timestamp: float            # Absolute MONOTONIC timestamp (seconds)
...
"""
Pure visual measurement instrument with absolute timestamping
...
def measure(  
    ...
    Args:  
        ...
        timestamps: (t0, t1) in MONOTONIC seconds, where t1 is measurement time  
          
    Returns:  
        List of Delta measurement records with absolute timestamps  
    ...
    time_window = measurement_time - before_time  
...
def measure_full_frame(  
    ...
    """  
    Full-frame reference measurement with absolute timestamp  
    ...
    return Delta(  
        ...
        measurement_reliability=self.max_reliability,  
        provenance={  
...
def _compute_perceptual_hash(self, image: np.ndarray) -> str:  
    """Noise-tolerant perceptual hash - low reliability"""  
    gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)  
    resized = cv2.resize(gray, (16, 16))  
    avg = resized.mean()  
    hash_bits = ['1' if resized[i, j] > avg else '0'   
...
def _cluster_pixels(self, diff_mask: np.ndarray) -> List[Tuple[int, int, int, int]]:  
    ...
    return self._merge_adjacent_regions(regions) if regions else []  
...
def _calculate_measurement_reliability(  
    ...
    """Calculate reliability based on signal strength and consistency ONLY"""  
